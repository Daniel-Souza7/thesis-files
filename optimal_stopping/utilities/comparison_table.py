...
def _write_table_for(
  ...
  if "EOP" in all_algos:
      ref_algo = "EOP"
      # Only apply ffill if it's a Series (multiple rows), not a scalar
  ...
  if ref_algo:
      ...
      print(df2)
      df2_with_index = df2.reset_index()  # FIXED: Don't use inplace, assign to new variable
      # FIXED: Check if column exists before trying to access it
